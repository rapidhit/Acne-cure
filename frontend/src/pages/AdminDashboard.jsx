...
export default function AdminDashboard() {
  ...
  async function handleSetDefault() {
    const updated = await api.adminSetDefaultProduct(selectedId);
    setProduct(updated);
    loadProductList();
  }
    ...
    <div className="min-h-screen bg-[#f9fafb] px-6 py-8">
      <div className="max-w-[1000px] mx-auto">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <button onClick={handleLogout} className="text-[13px] font-semibold text-[#0f3d1f]/70 underline">
            Log out
          </button>
        </div>
        ...
        <div className="mt-5 flex items-center gap-3 flex-wrap">
          <select
            ...
            className="rounded-full border border-black/10 bg-white px-4 py-2 text-[14px] font-semibold text-[#0f3d1f]"
          >
            {productList.map((p) => (
              <option key={p.id} value={p.id}>
                {p.name} (/{p.slug}){p.is_default ? " — default" : ""}
              </option>
            ))}
          </select>
          <button
            onClick={() => setShowNewProduct(true)}
            className="text-[13px] font-semibold text-[#0f3d1f] border border-[#0f3d1f]/20 rounded-full px-4 py-2 hover:bg-[#0f3d1f]/5"
          >
            + New Product
          </button>
          {product && !product.is_default && (
            <button onClick={handleSetDefault} className="text-[12px] text-[#0f3d1f]/60 underline">
          ...
          <>
            ...
            <div className="mt-6 rounded-2xl border border-black/10 bg-white p-5">
              ...
              <p className="text-[12px] text-[#0f3d1f]/60 mb-4">
                Live at yourdomain.com/{product.slug} — changes apply immediately.
              </p>
            ...
            {/* Stats */}
            {statsError && <p className="mt-6 text-[#dc2626]">{statsError}</p>}
            {stats && (
              <>
